...
namespace Platformer2D
{
    ...
    class Level : IDisposable
    {
        ...
        public Level(IServiceProvider serviceProvider, Stream fileStream, int levelIndex)
        {
            ...
            // Load background layer textures. For now, all levels mu
        ...
        private Tile LoadTile(char tileType, int x, int y)
        {
            switch (tileType)
            {
                ...
                //loads level door
                case 'L':
                    return loadLevelTile(x,y);
                //load core area door
                case 'C':
                    return loadCoreTile(x,y);
                //loads subarea door
                case 'S':
                    return loadSubTile(x,y);
                //loads mic. area door
                case 'M':
                    return loadMicTile(x,y);
                */
        ...
        private Tile LoadEnemyTile(int x, int y, string spriteSet)
        {
            ...
            enemies.Add(new Enemy(this, position, spriteSet,100));
        ...
        private void UpdateEnemies(GameTime gameTime)
        {
            ...
            foreach (Enemy enemy in enemies)
            {
                ...
                player.shootEnemy(enemy);
                
                // Touching an enemy instantly kills the player
                if (enemy.BoundingRectangle.Intersects(Player.BoundingRectangle) && !player.Invulernable)
                {
